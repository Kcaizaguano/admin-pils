...
export class NuevoRepuestoComponent implements OnInit {
  ...
  public f: FormGroup = this.form.group({
    ...
    codPils: [
      '',
      {
        validators: [
          Validators.required,
          Validators.pattern('[0-9a-zA-ZáéíóúñÁÉÍÓÚÑ /-]*'),
        ],
  ...
  ===========================================*/
  get numeroParte() {
    return this.f.get('numeroParte');
  }
  get presentacion() {
    return this.f.get('presentacion');
  }
  get nombre() {
    return this.f.get('nombre');
  }
  get precio() {
    return this.f.get('precio');
  }
  get codPils() {
    return this.f.get('codPils');
  }
  get almacen() {
    return this.f.get('almacen') as any;
  }
  get stockMinimo() {
    return this.f.get('stockMinimo');
  }
  get imagen() {
    return this.f.get('imagen');
  }
  ...
  async cargarListas() {
    ...
    this.productosService.getData().subscribe((resp) => {
  ...
  initForm() {
    this.f
      .get('nombre')!
      .valueChanges.pipe(
        debounceTime(500), // espera prudente
        distinctUntilChanged(), // evita repetir texto
        filter((valor) => valor && valor.length >= 3), // mínimo 3 letras
        switchMap((valor) =>
          this.productosService.buscarRepuestoPorNombre(valor),
        ),
      )
      .subscribe((resp) => {
        this.opcionesRepuestos = resp.data;
      });
  }

  filterData(resp: any, opcion: string) {
    if (opcion === 'nombre') {
      this.filterOptions = this.productosListado.filter((producto) =>
        producto.proNombre.toUpperCase().includes(resp),
      );
    } else {
  ...
  async guardar() {
    ...
    this.productosService.postData(dataProducto).subscribe((resp) => {
      if (resp.exito === 1) {
        this.loadData = false;
        alerts
          .saveAlert('Ok', resp.mensaje, 'success')
          .then(() => this.router.navigate(['/repuestos']));
      } else {
        this.loadData = false;
  ...
  codigoPilsRepetido() {
    return (control: AbstractControl) => {
      ...
      return this.productosService.validarCodigo(control.value, 1).pipe(
        map((resp) => {
          return resp.data ? { codigoPilsRepetido: true } : null;
        }),
        catchError(() => of(null)),
      );
    };
  ...
  /*=======================================
  Función para validar numero parte repetido
  ==========================================*/
  numeroParteRepetido() {
    return (control: AbstractControl) => {
      if (!control.value) {
      ...
      return this.productosService.validarCodigo(control.value, 2).pipe(
        map((resp) => {
          return resp.data ? { numeroParteRepetido: true } : null;
        }),
        catchError(() => of(null)),
      );
    };
