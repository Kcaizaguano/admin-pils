...
===========================================*/
  ...
  getFilterData(numElement: boolean) {
    ...
    this.productosService.getFilterData(filtroProductos).subscribe((resp) => {
      this.productos = Object.keys(resp.data).map(
        (a) =>
          ({
            ...
            nombreCompleto:
              resp.data[a].proNombre +
              ' ' +
              functions.obtenerPorPropiedad(resp.data[a].marca, 'marNombre') +
              ' ' +
              functions.obtenerPorPropiedad(resp.data[a].modelo, 'modNombre'),
          }) as Iproducto,
      );
        ...
        (a) =>
          ({
            ...
            nombreCompleto:
              resp.data[a].proNombre +
              ' ' +
              functions.obtenerModeloID(resp.data[a].modelos),
          }) as Iproducto,
      );
    ...
    alerts
      ...
      .then((result) => {
        if (result.isConfirmed) {
          this.productosService.deleteData(producto.proId).subscribe((resp) => {
            if (resp.exito === 1) {
              if (
                producto.proUrlImagen != null ||
                producto.proUrlImagen != ''
              ) {
                this.imagenesService
                  .deleteImage(
                    'Product',
                    functions.nombreImagen(producto.proUrlImagen, 'Product'),
                  )
                  .subscribe((resp) => {});
              }
